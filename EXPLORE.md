...
litprompt lock <file.md|dir/>                    # fetch remote imports via git, write prompt.lock
litprompt check <file.md|dir/>                   # validate imports resolve, lockfile is current, no cycles
```

The lockfile (`prompt.lock`) is discovered from the current working directory. Remote imports are fetched via git (HTTPS with SSH fallback) and cached in `~/.cache/litprompt/`.
