...
litprompt check                                    # validate every source in litprompt.yaml
...
`litprompt check` takes the same config-driven path as `build`: run it with no argument to validate every source in `litprompt.yaml`, or pass `--config <path>` to validate a named one. It resolves the config exactly as `build` does (same directory and lockfile semantics) but runs each source through the builder **without writing output** — so you can ask "will this environment build?" in CI before committing to it. Errors are collected and reported per source, and the command exits non-zero if any source fails to validate:

```sh
litprompt check                              # validate the discovered litprompt.yaml
litprompt check --config litprompt.prod.yaml # validate a named environment
litprompt check --config litprompt.prod.yaml --vars prod.env  # …including its variables
```

As with `build`, `--config` cannot be combined with a source argument, and CLI flags like `--match` are ignored in config mode (the source set comes from the config).
