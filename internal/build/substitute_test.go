...
func TestSubstituteVars(t *testing.T) {
	cases := []struct {
		...
		{
			name:         "lowercase var name reported as miscased",
			input:        "[{{X}}](#bot_id)\n",
			vars:         map[string]string{"BOT_ID": "U999"},
			want:         "[{{X}}](#bot_id)\n",
			wantMiscased: []string{"bot_id"},
		},
		{
			name:  "ordinary markdown link not treated as a variable",
			input: "See [the intro](#introduction) for details.\n",
			vars:  map[string]string{},
			want:  "See [the intro](#introduction) for details.\n",
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, missing, miscased := SubstituteVars(tc.input, tc.vars)
			if got != tc.want {
				t.Errorf("output\n  got:  %q\n  want: %q", got, tc.want)
			}
			...
			if len(miscased) != 0 || len(tc.wantMiscased) != 0 {
				if !reflect.DeepEqual(miscased, tc.wantMiscased) {
					t.Errorf("miscased = %v, want %v", miscased, tc.wantMiscased)
				}
			}
		})
	}
